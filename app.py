...
def upload():
    try:
        ...
        for name in matched_names[:10]:
            ...
            # convert to 8-bit for web display but keep 16-bit npy for backend
            label_vis = (label_map % 256).astype(np.uint8)

            label_name = f"{name}_label.png"
            label_path = os.path.join(UPLOAD_DIR, label_name)
            Image.fromarray(label_vis, mode="L").save(label_path)
